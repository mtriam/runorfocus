...
mapfile -t rows < <(echo "$clients_data" |
  jq -r --arg appid "$APPID" --arg ignore_title "$IGNORE_TITLE" '.clients[] | select(.appid==$appid and ($ignore_title=="" or ((.title|ascii_downcase) | contains(("" + $ignore_title)|ascii_downcase) | not))) | [.appid, .id, (.tags[0]//""), (.is_focused|tostring)] | @tsv')
